...
void test_queue__init(void) {
  ...
  // Testing the parameter passed into queue__init ??
  ...
  for (int i = 0; i < 100; i++) {
    TEST_ASSERT_EQUAL_UINT8(0, test_obj_queue.queue_memory[i]);
  }
}
...
void test_comprehensive(void) {
  ...
  for (size_t item = 0; item < max_queue_size; item++) {
    ...
    TEST_ASSERT_TRUE(queue__push(&test_obj_queue, item_pushed));
    TEST_ASSERT_EQUAL(item + 1, queue__get_item_count(&test_obj_queue));
  }

  // Should not be able to push anymore
  TEST_ASSERT_FALSE(queue__push(&test_obj_queue, 123));
  TEST_ASSERT_EQUAL(max_queue_size, queue__get_item_count(&test_obj_queue));
  ...
  for (size_t item = 0; item < max_queue_size; item++) {
    ...
    TEST_ASSERT_TRUE(queue__pop(&test_obj_queue, &popped_value));
    TEST_ASSERT_EQUAL(100 - (item + 1), queue__get_item_count(&test_obj_queue));
    TEST_ASSERT_EQUAL((uint8_t)item, popped_value);
  }

  // Test wrap-around case
  const uint8_t pushed_value = 123;
  TEST_ASSERT_TRUE(queue__push(&test_obj_queue, pushed_value));
  uint8_t popped_value = 0;
  TEST_ASSERT_TRUE(queue__pop(&test_obj_queue, &popped_value));
  TEST_ASSERT_EQUAL(pushed_value, popped_value);

  TEST_ASSERT_EQUAL(0, queue__get_item_count(&test_obj_queue));
  TEST_ASSERT_FALSE(queue__pop(&test_obj_queue, &popped_value));
}